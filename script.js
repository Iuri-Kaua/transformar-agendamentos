...
async function handleAppointmentSubmit(event) {
    ...
    try {
        ...
        // Simular envio para o backend (será implementado na próxima fase)
        const result = await submitAppointment(appointmentData);
        
        if (result.success) {
            showSuccessModal();
            event.target.reset();
            generateTimeSlots(); // Recarregar horários
        } else {
...
async function submitAppointment(appointmentData) {
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            resolve({
                success: success,
                message: success ? 'Agendamento realizado com sucesso!' : 'Erro no servidor. Tente novamente.'
            });
        }, 2000);
    });
}
